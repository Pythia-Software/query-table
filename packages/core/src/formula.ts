...
export type FormulaType = Exclude<FieldType, "enum"> | "null";
...
export type FormulaNode =
  | { kind: "literal"; value: FormulaValue; from: number; to: number }
  | {
      kind: "field";
      name: string;
...
export interface RegexInspection {
  input: string;
  start: number;
  end: number;
  groups: (string | null)[];
...
const f = (
  ...
  min: number,
  max: number,
  result: FormulaFunction["result"],
  args: FormulaFunction["args"],
): FormulaFunction => ({
  name,
  signature,
  description,
...
export const FORMULA_FUNCTIONS: FormulaFunction[] = [
  ...
  ...["LOWER", "UPPER", "TRIM", "LTRIM", "RTRIM"].map((n) =>
    f(
      n,
      `${n}(text)`,
  ...
  ...["CONTAINS", "STARTS_WITH", "ENDS_WITH"].map((n) =>
    f(
      n,
  ...
  ...["ABS", "FLOOR", "CEIL", "TRUNC", "SQRT"].map((n) =>
    f(n, `${n}(number)`, "Numeric transformation.", 1, 1, "number", ["number"]),
  ),
  f(
    "ROUND",
    "ROUND(number, digits)",
  ...
  ...["YEAR", "MONTH", "DAY", "HOUR", "WEEKDAY"].map((n) =>
    f(
      n,
      `${n}(datetime)`,
...
interface Token {
  text: string;
  kind: "field" | "string" | "number" | "word" | "op" | "end";
  from: number;
  to: number;
}
export function compileFormula(
  ...
  const call = (
    ...
    from: number,
    to: number,
  ): FormulaNode => ({ kind: "call", name, args, from, to });
  ...
  function expression(min = 0): FormulaNode {
    if (++depth > 50)
  ...
  const same = (ts: FormulaType[], n: FormulaNode): FormulaType => {
  ...
  function check(n: FormulaNode, level = 0): FormulaType {
    if (level > 50)
...
export function formulaRuntime(
  ...
  const str = (v: FormulaValue): string =>
    typeof v === "string" ? v : fail("Expected text.");
  const arr = (v: FormulaValue): string[] =>
    Array.isArray(v) && v.every((x) => typeof x === "string")
      ? v
      : fail("Expected a text array.");
  const integer = (v: FormulaValue, min = 0, max = 100000): number => {
    const n = num(v);
  ...
  const date = (v: FormulaValue): Date => {
    const s = str(v);
    if (
  ...
  const bound = (v: FormulaValue): FormulaValue => {
    if (
      v !== null &&
      typeof v !== "number" &&
      typeof v !== "string" &&
  ...
  let inspection: RegexInspection | undefined;
  let budget = 5000;
  function run(n: FormulaNode): FormulaValue {
    ...
    if (name === "op:AND" || name === "op:OR") {
      const a = run(args[0]!);
      if (name === "op:AND" && a === false) return false;
      if (name === "op:OR" && a === true) return true;
      const b = run(args[1]!);
      return name === "op:AND"
        ? b === false
          ? false
    ...
    switch (name) {
      ...
      case "LPAD":
      case "RPAD": {
        const chars = Array.from(str(a)),
          len = integer(b),
          pad = Array.from(vs.length > 2 ? str(c) : " ");
      ...
      case "STARTS_WITH":
      case "ENDS_WITH": {
        const s = c === true ? str(a).toLowerCase() : str(a),
          t = c === true ? str(b).toLowerCase() : str(b);
        out =
          name === "CONTAINS"
            ? s.includes(t)
            : name === "STARTS_WITH"
      ...
      case "REGEX_REPLACE": {
        ...
        if (name === "REGEX_TEST") out = re.test(str(a));
          ...
          let produced = 0;
          out = str(a).replace(re, (...captures: unknown[]) => {
            const replacement = str(c).replace(
              ...
              (_token, key: string) => {
                ...
                const extra =
                  typeof captures[captures.length - 1] === "object" ? 3 : 2;
                const i = Number(key);
            ...
            if (produced > 100000) fail("Replacement result is too large.");
            return replacement;
          });
        } else {
      ...
      case "DATE_ADD": {
        const d = date(c),
          unit = str(a).toLowerCase(),
          amount = integer(b, -100000, 100000);
